fn shade_planet_surface(rec: HitRecord, sphere: Sphere, sphere_pos_relative: vec3<f32>, ray: Ray, scene: SceneUniforms, camera: CameraUniforms) -> vec3<f32> {
    ...
    if (has_atmosphere) {
        ...
        let sky_ambient = get_sky_color(
            ...
            sphere_pos_relative, light_dir_to_source, light_color, scene, INFINITY,
            &shadow_casters, shadowParams, camera, self_pos_world
        );
        ...
        if (dot_nl > 0.0 && !is_eclipsed) {
             ...
             let world_scale_inner = scene.scale_and_flags.x;
             ...
             let optical_lengths = calculate_optical_length_to_space(
                 ...
                 params.h_rayleigh, params.h_mie, world_scale_inner
             );
             let optical_depth = (params.beta_rayleigh * optical_lengths.x + params.beta_mie * optical_lengths.y) * world_scale_inner;
